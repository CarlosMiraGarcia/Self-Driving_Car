...
is
   ...
   begin
      ...
      Put_Line ("");
      Put_Line (HT & ESC & "[102m" & " s " & ESC & "[0m" &
         " to (s)tart/(s)top the car." & HT & HT & "        _______");
      Put_Line (HT & ESC & "[102m" & " p " & ESC & "[0m" &
         " to change gear to (p)arked." & HT & "               //  ||\ \");
      Put_Line (HT & ESC & "[102m" & " f " & ESC & "[0m" &
         " to change gear to (f)orward." & HT & " _____//___||_\ \___");
      Put_Line (HT & ESC & "[102m" & " r " & ESC & "[0m" &
         " to change gear to (r)eversing." & HT & " )  _          _    \");
      Put_Line (HT & ESC & "[102m" & " c " & ESC & "[0m" & " to (c)harge the car." &
         HT & HT & " |_/ \________/ \___|");
      Put_Line (HT & ESC & "[102m" & " d " & ESC & "[0m" &
   ...
   procedure PrintHeader (dummy_car : in out Car) is
   begin
      ...
      if dummy_car.carBattery.charging = Off then
         Put_Line (HT & HT & HT & "Charging Status: " & ESC & "[1;30m" & ESC &
            "[101m" & " " & dummy_car.carBattery.charging'Image & " " & ESC & "[0m");
      elsif dummy_car.carBattery.charging = On then
         Put_Line (HT & HT & HT & "Charging Status: " & ESC & "[1;30m" & ESC &
            "[102m" & " " & dummy_car.carBattery.charging'Image & " " & ESC & "[0m");
      end if;
      ...
      if dummy_car.baterryPer >= 0.5 then
         Put_Line (HT & HT & HT & "Battery Status: " & ESC & "[1;30m" & ESC &
            "[102m" & Integer (dummy_car.carBattery.charge / BatteryCharge (10))'Image &
            "% " & ESC & "[0m");
      elsif (dummy_car.baterryPer >= 0.1 and dummy_car.baterryPer < 0.5) then
         Put_Line (HT & HT & HT & "Battery Status: " & ESC & "[1;30m" & ESC & "[43m" &
            Integer (dummy_car.carBattery.charge / BatteryCharge (10))'Image &
            "% " & ESC & "[0m");
      else
         Put_Line (HT & HT & HT & "Battery Status: " & ESC & "[1;30m" & ESC &
            "[101m" & Integer (dummy_car.carBattery.charge / BatteryCharge (10))'Image &
            "% " & ESC & "[0m");
      end if;
      Put_Line ("");
