...
is
   ...
   begin
      Put_Line
        (HT &
         "_MENU_________________________________________________________");
   ...
   procedure PrintHeader (dummy_car : in Car) is
   begin
      Put_Line
        (HT &
         "__STATUS______________________________________________________");
      ...
      if dummy_car.carBattery.batteryPer >= 50.0 then
         Put_Line
           (HT & HT & HT & "Battery Status:  " & ESC & "[1;30m" & ESC &
            "[102m" &
            Integer (dummy_car.carBattery.currentCharge / BatteryCharge (10))'
              Image &
            "% " & ESC & "[0m");
      elsif
        (dummy_car.carBattery.batteryPer >= 10.0 and
         dummy_car.carBattery.batteryPer < 50.0)
      then
         Put_Line
           (HT & HT & HT & "Battery Status:  " & ESC & "[1;30m" & ESC & "[43m" &
            Integer (dummy_car.carBattery.currentCharge / BatteryCharge (10))'
              Image &
            "% " & ESC & "[0m");
      else
      ...
      Put_Line
        (HT &
         "__DRIVING_____________________________________________________");
      Put
      ...
      Put_Line (HT & "      Steering Wheel: " & dummy_car.steeringWheel'Image);
      Put_Line ("");
   end PrintHeader;
   ...
   begin
      Move (StringInput (StringInput'First .. Pos), FirstHalf);
      Move
        (StringInput (Pos + CarPrint'Length + 1 .. StringInput'Last),
         SecondHalf);
   ...
   is
      FoundChar : String (1 .. 1);
   begin
      Move (StringInput (Pos .. Pos), FoundChar);
      return FoundChar;
   end GetChar;
